...
import pufferlib
from pufferlib.ocean.viz_breakout import binding

class VizBreakout(pufferlib.PufferEnv):
    def __init__(self, num_envs=1, render_mode=None,
        ...
        self.single_observation_space = gymnasium.spaces.Box(low=0, high=1,
            shape=((height // 4) * (width // 4),), dtype=np.float32)
...
def test_pixel_behavior(timeout=10, atn_cache=1024, save_dir="frames", gif_name="output.gif"):
    import numpy as np
    ...
    while tick < 10:
        atn = actions[tick % atn_cache]
        obs, reward, term, trunc, info = env.step(atn)
        obs = obs.reshape(env.height // 4, env.width // 4)
        print(obs.shape, obs.mean())
        img_array = (obs * 255).astype(np.uint8)
        img = Image.fromarray(img_array, mode='L')
        filename = os.path.join(save_dir, f"frame_{tick:04d}.png")
        img.save(filename)
        frames.append(img)
